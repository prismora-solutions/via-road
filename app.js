...
badges.innerHTML += ` <div style="width:100%; margin-top:8px;"> <div class="progress-label">✅ Checklist : ${faits}/${total}</div> <div class="progress-bar-wrap"> <div class="progress-bar" id="progress-bar" style="width:${pct}%"></div> </div> </div>`;
}

// ===== NAVIGATION JOURS =====
function rendreNavJours() {
const nav = document.getElementById(‘nav-jours’);
nav.innerHTML = SEJOUR.jours.map((j, i) => `<button class="nav-jour-btn ${i === jourActif ? 'actif' : ''}" onclick="changerJour(${i})"> <span class="nav-emoji">${j.emoji}</span> <span class="nav-label">J${j.numero}</span> <span class="nav-label" style="font-weight:400;opacity:.7">${j.label.split(' ').slice(0,2).join(' ')}</span> </button>`).join(’’);
}
...
let html = ` <div class="jour-header"> <div class="jour-emoji-grand">${jour.emoji}</div> <div> <div class="jour-info-label">${jour.label}</div> <div class="jour-info-titre">${jour.titre}</div> </div> </div>`;
...
// Liens alternatifs (Visorando, Terra Aventura, options A/B…)
...
return ` <div class="etape-card type-${etape.type}"> <div class="etape-accent"></div> <div class="etape-icone">${icone}</div> <div class="etape-corps"> <div class="etape-heure">${etape.heure}</div> <div class="etape-titre">${etape.titre}</div> <div class="etape-desc">${etape.description}</div> ${footer} </div> </div>`;
}
...
html += ` <div class="checklist-item ${fait ? 'fait' : ''}" onclick="toggleCheck(${item.id})"> <input type="checkbox" id="check-${item.id}" ${fait ? 'checked' : ''} onchange="toggleCheck(${item.id})" onclick="event.stopPropagation()"> <label for="check-${item.id}">${item.texte}</label> </div>`;
});
});
